...
@app.route("/registration_analytics")
def registration_analytics():
    # Query deliveries from MongoDB
    deliveries = list(deliveries_col.find())

    # Overall Summary
    total_registrations = len(deliveries)
    successful = sum(1 for d in deliveries if d.get("status") == "successful")
    unsuccessful = sum(1 for d in deliveries if d.get("status") == "unsuccessful")
    pending = total_registrations - successful - unsuccessful

    # Daily Registration Trends (last 30 days)
    today = datetime.utcnow().date()
    last_30_days = [today - timedelta(days=i) for i in reversed(range(30))]
    daily_registrations = {d.strftime('%Y-%m-%d'): 0 for d in last_30_days}
    for d in deliveries:
        ts_str = d.get("timestamp")
        if ts_str:
            try:
                ts_date = datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S").date()
                ts_key = ts_date.strftime('%Y-%m-%d')
                if ts_key in daily_registrations:
                    daily_registrations[ts_key] += 1
            except Exception:
                continue
    ...
    # Location Analysis (count by pickup location and registration %)
    location_counts = Counter(d.get("pickup", "Unknown") for d in deliveries)
    location_percentages = {}
    for loc, count in location_counts.items():
        location_percentages[loc] = (count / total_registrations * 100) if total_registrations > 0 else 0

    # Last 7 Days Registration Trend
    last_7_days = [today - timedelta(days=i) for i in reversed(range(7))]
    last_7_days_registrations = {d.strftime('%Y-%m-%d'): 0 for d in last_7_days}
    for d in deliveries:
        ts_str = d.get("timestamp")
        if ts_str:
            try:
                ts_date = datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S").date()
                ts_key = ts_date.strftime('%Y-%m-%d')
                if ts_key in last_7_days_registrations:
                    last_7_days_registrations[ts_key] += 1
            except Exception:
                continue
...
@app.route("/registration_analytics/export_pdf")
def export_registration_analytics_pdf():
    # Render the same template but for PDF (could reuse same or create PDF-specific template)
    # Reuse logic from above or query again (for simplicity, reusing the logic here)
    ...
    rendered = render_template("registration_analytics_pdf.html",
                               total_registrations=total_registrations,
                               successful=successful,
                               unsuccessful=unsuccessful,
                               pending=pending,
    ...
    pdf_file.seek(0)
    return send_file(pdf_file, download_name="registration_analytics_report.pdf", as_attachment=True)
